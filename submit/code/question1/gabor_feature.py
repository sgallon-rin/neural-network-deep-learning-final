...
def feature_set(S):
    ...
    for k in range(len(S)):
        ...
        # index = [2, 3, 4, 5, 6, 8, 9, 21, 22, 24, 27, 28] # for T
        # index = [1,7,10,20,23,25,26] # for D
        index = [11, 12, 13, 14, 15, 16, 17, 18, 19]  # for N
        pickle.dump(single_S, open('data_parts_g/N_' + str(index[k//4]) + '_'+str(k%4+1) +'.p', 'wb'))
...
if __name__ == '__main__':
    ...
    pickle.dump(np.array(T), open('data_parts_g/tight_frame_T.p', 'wb'))
    pickle.dump(np.array(N), open('data_parts_g/tight_frame_N.p', 'wb'))
    pickle.dump(np.array(D), open('data_parts_g/tight_frame_D.p', 'wb'))
